...
void Grid::Shoot_around(char column_c, int row)
{
	int column = (int)column_c - (int)'a';
	row--;
	int tile_number = 10*row + column;
	if (row != 0)
	{
		if(this->tiles[tile_number - 10].state != Tile::shot) this->tiles[tile_number - 10].state = Tile::free;
		if (column != 0)
		{
			this->tiles[tile_number - 11].state = Tile::free;
			if(this->tiles[tile_number - 1].state != Tile::shot)this->tiles[tile_number - 1].state = Tile::free;
		}
		if (column != 9)
		{
			this->tiles[tile_number - 9].state = Tile::free;
			if(this->tiles[tile_number + 1].state != Tile::shot)this->tiles[tile_number + 1].state = Tile::free;
		}
	}
	if (row != 9)
